...
class PlayerShip(Ship):
    ...
    def shoot(self):
        if self.cool_down_counter == 0:
            laser = Laser(self.x+20, self.y-17, self.laser_img)
            self.lasers.append(laser)
...
class EnemyShip(Ship):
    ...
    def shoot(self):
        if self.cool_down_counter == 0:
            laser = Laser(self.x+5, self.y+17, self.laser_img)
            self.lasers.append(laser)
    ...
    while run:
        ...
        if len(enemies) == 0:
            ...
            for i in range(enemy_length):
                enemy = EnemyShip(random.randrange(1, 680),
        ...
        if keys[pygame.K_RIGHT] and player.x < 665:
            player.x += player_velocity
